...
var MenuBtnComponent = React.createClass({
    ...
    render:function(){
        ...
        return <BtnComponent style = {{backgroundColor:'#9FAA83'}} className = 'stateVector' handleClick = {this.handleClick}><span>Menu</span>{(menuHint>0?<span className = 'badge'>{menuHint}</span>:'')}</BtnComponent>
    }
});
    ...
    render:function(){
        ...
        function getSkillList(){
            if(getLength(skill)==0){
                return <p style = {{color:COLOR.YELLOW}}>You haven't learn any skill yet XD</p>
            }
            var result = [];
                ...
                return (
                    <div className = 'skillMenu'>
                        <div className = 'skill panel panel-primary'>
                            <div className = 'panel-heading'>All skills</div>
                ...
                return (
                    <div className = 'skillMenu'>
                        <div style = {{marginTop:10}}>
                            <div>
                                <label >Author ---> Eric Liu</label>
                                <label htmlFor="account">(Save)</label>
                            ...
                            <div>
                                {this.context.currentScene != 'home'?<p style = {{color:'#ddd'}}>You can only save when you at home</p>:null}
                                {getLength(this.context.mstState) != 0?<p style = {{color:'#ddd'}}>You can't save during the fight</p>:null}
                                {this.context.robberSaveData.robber?<p style = {{color:'#ddd'}}>You are in danger</p>:null}
                                <BtnComponent disabled = {this.context.currentScene != 'home' || (getLength(this.context.mstState) != 0)||(this.context.robberSaveData.robber)} handleClick = {this.willUpload}>Export</BtnComponent>
                                <BtnComponent handleClick = {this.download}>Import</BtnComponent>
                            </div>
                        </div>
                        ...
                        <label className = "checkbox" htmlFor="autoSave">
                            ...
                            <span>Save when goto explore</span>
                        </label>
                        <BtnComponent handleClick = {this.context.setVolume}>Sound：{this.context.AudioEngine.on?'On':'Off'}</BtnComponent>
                        <BtnComponent handleClick = {this.setSort}>Auto sort inventory：{this.context.settings.sort?'On':'Off'}</BtnComponent>
                        <div><a target="blank" href = "http://1.maou.sinaapp.com/?page_id=47"></a></div>
                    </div>
                )
            }
        }
        return  <div className = 'menuOuter'>
                    <div className = 'menuInner'>
                        <div className = 'menu'>
                            ...
                            <ul className="nav">
                                <div className='btn' onClick = {this.handleTab.bind(this,'skill')}>Skill</div>
                                <div className='btn' onClick = {this.handleTab.bind(this,'settings')}>Setting</div>
                            </ul>
                            <BtnComponent desc = 'Back' handleClick = {this.quitMenu}/>
                        </div>
                    </div>
                </div>
    }
    ...
    render:function(){
        ...
        var getRow = function(){
            ...
            for (var attr in buildingList) {
                ...
                result.push(<tr key = {count}>
                                ...
                                <td>{data.desc}</td>
                                <td>{data.timeNeed}</td>
                                <td><BtnComponent desc = {'Build'} disabled = {maxTimeToUse <= data.timeNeed} requireList = {data.require} handleClick = {this.build.bind(this,attr)}/></td>
                            </tr>);
                count ++;
        ...
        return  <div>
                    <div  className = "tableOuter buildTable">
                        <table className = "table table-condensed table-hover">
                            <thead><tr><td>Build</td><td>Require</td><td>Desc</td><td>Time/td><td></td></tr></thead>
                            <tbody>
                                {getRow.bind(this)()}
                            </tbody>
                        </table>
...
var HomeComponent = React.createClass({
    ...
    render:function(){
        ...
        return <div>
                    ...
                    <div>
                        <BtnComponent desc = 'Explore' handleClick = {this.handleGoOut}/>
                    </div>
                </div>
    }
});
        ...
        function getPlaceDisplay(){
            ...
            for (var attr in PLACE_DATA) {
                ...
                result.push(<tr key = {attr}>
                                 ...
                                 <td><BtnComponent desc = 'GO' handleClick = {this.handleGo.bind(this,attr)} /></td>
                            </tr>);
            };
            return result;
        };
...
var MainComponent = React.createClass({
    ...
    download:function(){
        ...
        element.charset = 'utf-8';
                            element.setAttribute('src',"./save/"+save_account+".js?t="+ Date.parse(new Date()));
                    ...
                    self.setState({saveData:Filedata});
    ...
    downloadFile(save_account+".js","var Filedata="+JSON.stringify(saveData))
